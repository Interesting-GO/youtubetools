...
func NewYoutube(debug bool, proxy string) *Youtube {
	return &Youtube{DebugMode: debug, DownloadPercent: make(chan int64, 100), Proxy: proxy}
}
...
func (y *Youtube) getVideoInfo() error {
	...
	var err error
	if y.Proxy != "" {
		resp, err = httplib.ProxyDow(url, y.Proxy)
	} else {
...
func (y *Youtube) videoDLWorker(destFile string, target string) error {
	...
	var err error
	if y.Proxy != "" {
		resp, err = httplib.ProxyDow(target, y.Proxy)
	} else {
		resp, err = httplib.Get(target).SetUserAgent(easyutils.ReptileGetUserAgent()).Response()
	}
